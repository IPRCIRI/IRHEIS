...
for(year in (Settings$startyear:Settings$endyear)){
  ...
  X <- MD[,.(Total_Exp_Month=weighted.mean(Durable_Sale-Durable_Exp,Weight,na.rm = TRUE)),by=c("Region","HSex","Decile")]
  X[,Year:=year]
  Data31 <- rbind(Data31,X)
  write.csv(Data31,file="Data31.csv")
  
  X <- MD[,.(Total_Exp_Month=weighted.mean(Durable_Sale-Durable_Exp,Weight,na.rm = TRUE)),by=c("HSex","Decile")]
  X[,Year:=year]
  Data32 <- rbind(Data32,X)
  write.csv(Data32,file="Data32.csv")
  
  X <- MD[,.(Total_Exp_Month=weighted.mean(Durable_Sale-Durable_Exp,Weight,na.rm = TRUE)),by=c("Decile")]
  X[,Year:=year]
  Data33 <- rbind(Data33,X)
  write.csv(Data33,file="Data33.csv")
  
  X <- MD[,.(Total_Exp_Month=weighted.mean(Durable_Sale-Durable_Exp,Weight,na.rm = TRUE)),by=c("Region","Decile")]
  X[,Year:=year]
