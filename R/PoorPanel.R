...
P<-as.data.table(cbind(p1$cluster3,p1$V1,p2$V1,p3$V1,p4$V1,p5$V1,p6$V1,p7$V1,p8$V1,p9$V1,
                       p10$V1,p11$V1,p12$V1,p13$V1,p14$V1,p15$V1,p16$V1,p17$V1,p18$V1,p19$V1,p20$V1,p21$N))
names(P)<-c("cluster3","Total_Exp_Month","FoodKCaloriesHH","FoodProteinHH","FoodExpenditure",
            ...
            "Transportation_Exp","Durable_Exp","Total_Exp_Month_nondurable","Medical_Exp","Year","size")
...
ggplot(data=PANEL,aes(x=Year, y=log(Total_Exp_Month), group=cluster3, colour=cluster3))+geom_line()
ggplot(data=PANEL,aes(x=Year, y=log(Medical_Exp), group=cluster3, colour=cluster3))+geom_line()
